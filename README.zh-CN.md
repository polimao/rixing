...
翻译和轻听音乐功能在**首次使用时**下载 AI 模型，之后**完全本地推理**：

- **翻译** —— HY-MT gguf 模型（~1.1 GB）首次使用时从 HuggingFace 下载，缓存在 App 数据目录（与待办同处 iCloud Drive / App Support）。
- **轻听** —— MRT2 模型（~0.5 GB）加一个 `uv` 管理的小型 Python 环境首次启动时下载，缓存在 `~/Documents/Magenta/magenta-rt-v2/`。

两者在 HuggingFace 主站慢/不可达时会自动回退 `hf-mirror.com` 镜像。下载完成后推理 100% 本地完成——你的音频、文本、待办数据都不会出本机。
